...
RSpec.describe Usuario do
	before(:example) do
		@user = Usuario.new
		@user.cadastrar "1234", "1234", "Cris", true, ["Mat", "Ial"]
		@user.cadastrar "123", "123", "Cris", false, ["Mat", "Ial"]
	end
	describe "#logar" do
	...
	describe "#cadastrar" do
		it 'passo usuario já existente e espero que falhe' do
			expect(@user.cadastrar "123", "123", "Cris", false, ["Mat", "Ial"]).to be false
		end
		it 'passo usuario não existente e espero que cadastre' do
			expect(@user.cadastrar "12", "12", "Cris", false, ["Mat", "Ial"]).to be true
		end
